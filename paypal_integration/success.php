...
if(!empty($txn_id)){
    ...
    $paymentResult = mysqli_query($con, "SELECT * FROM payments WHERE txn_id = '".$txn_id."'");
    if($paymentResult->num_rows > 0){
        ...
        //order items details
        $orderItemResult = mysqli_query($con, "SELECT p.name, i.quantity, i.gross_amount FROM order_items as i LEFT JOIN products as p ON p.id = i.item_number WHERE payment_id = '".$payment_id."'");
?>
