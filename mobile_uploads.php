...
***********************************************************/
	if (!isset($_POST['x']) && !isset($_FILES['image']['name']) ){
		//Delete users temp image
			$temppath = 'assets/images/mobile_uploads/'.$profile_id.'_temp.jpeg';
...
***********************************************************/	
	...
	//Set Upload directory    
		$uploaddir = $_SERVER['DOCUMENT_ROOT'].'/phpcourse/assets/images/mobile_uploads';
	//Set File name	
	...
	//Move the file to correct location
		...
		if (!$move) { 
			...
			$imgSrc= "assets/images/mobile_uploads/".$file_name; // the image to display in crop area
...
if (isset($_POST['x'])){
	...
	//the file type posted
		$type = $_POST['type'];	
	//the image src
		$src = 'assets/images/mobile_uploads/'.$_POST['src'];	
	...
	if($type == 'jpg' || $type == 'jpeg' || $type == 'JPG' || $type == 'JPEG'){	
		...
		//save the new cropped version
			imagejpeg($dst_r, "assets/images/mobile_uploads/".$finalname."n.jpeg", 90); 	
		...
		$result_path ="assets/images/mobile_uploads/".$finalname."n.jpeg";

		//Insert image into database
		$insert_pic_query = mysqli_query($con, "UPDATE users SET images='$result_path' WHERE username='$userLoggedIn'");
...
<div id="uploadpage">
	...
	<div id="formExample">
	    ...
	    <form action="mobile_uploads.php" method="post"  enctype="multipart/form-data">
    ...
    if($imgSrc){ //if an image has been uploaded display cropping area?>
	    ...
	    <div id="CroppingContainer" style="width:800px; max-height:600px; background-color:#FFF; position:relative; top:50px; left:300px; overflow:hidden; border:2px #666 solid; z-index:2001; padding-bottom:0px;">  
	        ...
	        <div id="CropImageForm" style="width:100px; height:30px; float:left; margin:10px 0px 0px 40px;" >  
	            <form action="mobile_uploads.php" method="post" onsubmit="return checkCoords();">
	        ...
	        <div id="CropImageForm2" style="width:100px; height:30px; float:left; margin:10px 0px 0px 40px;" >  
	            <form action="mobile_uploads.php" method="post" onsubmit="return cancelCrop();">
